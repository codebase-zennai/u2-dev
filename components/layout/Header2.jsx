...
export default function Header2({ isSolid = false }) {
  ...
  return (
    <div className="header2-nav">
      ...
      />
      <div
        className={`nav_wrapper fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isSolid || isScrolled ? "is-scrolled" : "is-at-top"}`}
        style={{
          transform: isVisible ? "translateY(0)" : "translateY(-100%)",
          transition:
            "transform 0.3s ease-in-out, background-color 0.3s ease-in-out, padding 0.3s ease-in-out",
        }}
      >
          ...
          <div className="nav_wrap flex items-center justify-between w-full max-w-7xl mx-auto px-4 py-3 md:px-8">
            ...
            >
              {!isAgent && (
                <Link
                  href="/register"
                  className="bg-[#dfa447] hover:bg-[#013b85] text-white px-5 py-2.5 rounded-full font-bold text-sm tracking-wide transition-all shadow-sm hover:shadow-md !no-underline inline-block"
                >
                  Sign Up
                </Link>
              )}
                ...
                {!isAgent && (
                  <div className="pt-4 border-t border-slate-100 w-full mt-4 flex flex-col gap-3">
                    <Link
                      href="/register"
                      onClick={toggleMenu}
                      className="bg-[#dfa447] hover:bg-[#013b85] text-white text-center w-full py-3.5 rounded-xl font-bold tracking-wide transition-all !no-underline block"
                    >
                      Sign Up
                    </Link>
                  </div>
