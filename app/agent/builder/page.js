...
export default function BuilderPage() {
  ...
  const [itinerary, setItinerary] = useState({
    client: { name: "", agency: "", adults: 2, children: 0, dates: "", style: "Standard" },
  ...
  const handleGenerateQuote = () => {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem("current_quote", JSON.stringify({ ...itinerary, total: calculateTotal() }));
      ...
      <div className="min-h-screen bg-slate-50 pt-20 flex flex-col font-sans">
        ...
        <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-20 z-30 shadow-sm">
          <div className="flex items-center gap-4">
            <Link href="/agent" className="text-slate-500 hover:text-slate-900 transition-colors">
              <ChevronLeft size={24} />
            </Link>
            <div>
              <h1 className="text-xl font-bold text-slate-900">Itinerary Builder</h1>
              <p className="text-xs text-slate-500 font-medium">Draft • {itinerary.client.name || "New Client"}</p>
            </div>
          </div>
          <div className="flex gap-3">
            ...
            >
              {isSaving ? "Generating..." : "Generate Quote"} <ChevronRight size={16} />
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-64 bg-white border-r border-slate-200 overflow-y-auto hidden md:block">
            <div className="p-4">
              <div className="text-xs font-black text-slate-400 uppercase tracking-wider mb-4 ml-2">Wizard Steps</div>
          ...
          <div className="flex-1 overflow-y-auto bg-slate-50/50 p-6 md:p-10 relative">
            <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-sm border border-slate-200 min-h-[600px] flex flex-col">
              ...
              <div className="flex-1 p-8">
                ...
                {currentStep === 1 && <Step1Client itinerary={itinerary} updateItinerary={updateItinerary} />}
                {currentStep === 2 && <Step2Destinations itinerary={itinerary} updateItinerary={updateItinerary} />}
                {currentStep === 3 && <Step3Accommodation itinerary={itinerary} updateItinerary={updateItinerary} />}
                {currentStep === 4 && <Step4Transportation itinerary={itinerary} updateItinerary={updateItinerary} />}
                {currentStep === 5 && <Step5Attractions itinerary={itinerary} updateItinerary={updateItinerary} />}
                {currentStep === 6 && <Step6Meals itinerary={itinerary} updateItinerary={updateItinerary} />}
                {currentStep === 7 && <Step7Services itinerary={itinerary} updateItinerary={updateItinerary} />}
                {currentStep === 8 && <Step8Timeline itinerary={itinerary} updateItinerary={updateItinerary} />}
                {currentStep === 9 && <Step9Costing itinerary={itinerary} total={calculateTotal()} />}
          ...
          <div className="w-72 bg-white border-l border-slate-200 hidden xl:flex flex-col">
            ...
            <div className="flex-1 p-6 overflow-y-auto flex flex-col gap-4">
              <div className="flex justify-between text-sm">
                <span className="text-slate-500 font-medium">Accommodations</span>
                <span className="font-bold text-slate-900">${itinerary.accommodations.reduce((a,b)=>a+(b.price||0),0)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500 font-medium">Transportation</span>
                <span className="font-bold text-slate-900">${itinerary.transportation.reduce((a,b)=>a+(b.price||0),0)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500 font-medium">Attractions</span>
                <span className="font-bold text-slate-900">${itinerary.attractions.reduce((a,b)=>a+(b.price||0),0)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500 font-medium">Extra Services</span>
                <span className="font-bold text-slate-900">${itinerary.services.reduce((a,b)=>a+(b.price||0),0)}</span>
              </div>
              <div className="my-2 border-t border-dashed border-slate-200"></div>
              <div className="flex justify-between items-end">
                <span className="text-xs font-black text-slate-400 uppercase tracking-wider">Est. Total</span>
...
const Step1Client = ({ itinerary, updateItinerary }) => {
  ...
  const updateClient = (field, val) => updateItinerary("client", { ...client, [field]: val });
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="flex flex-col gap-2">
        ...
        <input 
          type="text" value={client.name} onChange={e => updateClient("name", e.target.value)}
      ...
      <div className="flex flex-col gap-2">
        <label className="text-xs font-bold text-slate-500 uppercase">Agency / Company</label>
        <input 
          type="text" value={client.agency} onChange={e => updateClient("agency", e.target.value)}
          className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium focus:border-blue-500 outline-none" 
          placeholder="e.g. Acme Corp"
        />
      </div>
      <div className="flex flex-col gap-2">
        <label className="text-xs font-bold text-slate-500 uppercase">Travel Dates</label>
        <input 
          type="text" value={client.dates} onChange={e => updateClient("dates", e.target.value)}
          className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium focus:border-blue-500 outline-none" 
          placeholder="12 Dec - 20 Dec 2026"
        />
      </div>
      <div className="flex flex-col gap-2">
        <label className="text-xs font-bold text-slate-500 uppercase">Travel Style</label>
        <select 
          value={client.style} onChange={e => updateClient("style", e.target.value)}
          className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium focus:border-blue-500 outline-none"
        >
      ...
      <div className="flex gap-4 md:col-span-2">
        <div className="flex-1 flex flex-col gap-2">
          ...
          <input 
            type="number" min="1" value={client.adults} onChange={e => updateClient("adults", parseInt(e.target.value))}
            className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium focus:border-blue-500 outline-none" 
          />
        </div>
        <div className="flex-1 flex flex-col gap-2">
          ...
          <input 
            type="number" min="0" value={client.children} onChange={e => updateClient("children", parseInt(e.target.value))}
            className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium focus:border-blue-500 outline-none" 
          />
        </div>
      </div>
...
const Step2Destinations = ({ itinerary, updateItinerary }) => {
  ...
  const removeDest = (id) => {
    updateItinerary("destinations", itinerary.destinations.filter(d => d.id !== id));
  };

  return (
    <div className="flex flex-col gap-8">
      <div>
        <h3 className="text-sm font-bold text-slate-900 mb-3">Available Destinations</h3>
        <div className="flex flex-wrap gap-2">
          {DESTINATIONS.map(d => (
            <button 
              key={d.id} onClick={() => addDest(d)}
      ...
      <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
        ...
        {itinerary.destinations.length === 0 ? (
          <p className="text-sm text-slate-400 font-medium">No destinations selected yet.</p>
        ) : (
          <div className="flex flex-col gap-3">
            {itinerary.destinations.map((d, idx) => (
              <div key={d.id} className="flex items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                ...
                <div className="flex-1 font-bold text-slate-900">{d.city} <span className="text-slate-400 font-medium">({d.country})</span></div>
                <button onClick={() => removeDest(d.id)} className="text-red-400 hover:text-red-600 p-2">
                  <Trash2 size={16} />
                </button>
              </div>
            ))}
...
const Step3Accommodation = ({ itinerary, updateItinerary }) => {
  ...
  const relevantHotels = HOTELS.filter(h => itinerary.destinations.some(d => d.id === h.cityId));
  ...
  return (
    <div className="flex flex-col gap-8">
      {itinerary.destinations.length === 0 && <p className="text-slate-500 text-sm">Please select a destination first.</p>}
      
      {relevantHotels.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {relevantHotels.map(hotel => (
            <div key={hotel.id} className="bg-white border border-slate-200 rounded-2xl overflow-hidden flex flex-col">
              ...
              <div className="p-4 flex-1 flex flex-col">
                ...
                <p className="text-xs text-slate-500 mt-1 mb-4 flex-1 line-clamp-2">{hotel.description}</p>
                
                <div className="flex gap-2">
                  <button 
                    onClick={() => addHotel(hotel, hotel.rooms[0], hotel.mealPlans[0])}
        ...
        <div className="bg-blue-50/50 p-6 rounded-2xl border border-blue-100 mt-4">
          ...
          <div className="flex flex-col gap-2">
            {itinerary.accommodations.map(a => (
              <div key={a.uniqueId} className="flex justify-between items-center bg-white p-3 rounded-xl border border-slate-200 text-sm">
                <div>
                  <span className="font-bold">{a.name}</span>
                  <span className="text-slate-500 ml-2">({a.selectedRoom} - {a.selectedPlan})</span>
                </div>
                <div className="flex items-center gap-4">
                  <span className="font-black text-blue-600">${a.price}</span>
                  <button onClick={() => {
                    updateItinerary("accommodations", itinerary.accommodations.filter(item => item.uniqueId !== a.uniqueId));
...
const Step4Transportation = ({ itinerary, updateItinerary }) => {
  const addTransport = (t) => {
    updateItinerary("transportation", [...itinerary.transportation, { ...t, price: t.basePrice, uniqueId: Math.random().toString() }]);
  };
    ...
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {TRANSPORT_OPTIONS.map(t => (
          <div key={t.id} className="border border-slate-200 p-4 rounded-xl bg-white hover:border-blue-300 transition-colors flex justify-between items-center">
            <div>
              <h4 className="font-bold text-slate-900">{t.type}</h4>
              <p className="text-xs text-slate-500 mt-1">Capacity: {t.capacity} Pax</p>
            </div>
            <div className="flex items-center gap-4">
              <span className="font-black text-blue-600">${t.basePrice}</span>
              <button onClick={() => addTransport(t)} className="w-8 h-8 rounded-full bg-slate-100 hover:bg-blue-100 text-blue-600 flex items-center justify-center">
                <Plus size={16} />
              </button>
            </div>
          </div>
        ...
        <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 mt-4">
          ...
          <div className="flex flex-col gap-2">
            {itinerary.transportation.map(t => (
              <div key={t.uniqueId} className="flex justify-between items-center bg-white p-3 rounded-xl border border-slate-200 text-sm">
                <span className="font-bold">{t.type}</span>
                <div className="flex items-center gap-4">
                  <span className="font-black text-slate-700">${t.price}</span>
                  <button onClick={() => {
                    updateItinerary("transportation", itinerary.transportation.filter(item => item.uniqueId !== t.uniqueId));
                  }} className="text-red-400 hover:text-red-600">
                    <Trash2 size={16} />
                  </button>
                </div>
...
const Step5Attractions = ({ itinerary, updateItinerary }) => {
  const addAttr = (a) => {
    updateItinerary("attractions", [...itinerary.attractions, { ...a, uniqueId: Math.random().toString() }]);
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {ATTRACTIONS.map(a => (
          <div key={a.id} className="border border-slate-200 p-4 rounded-xl bg-white hover:shadow-md transition-all flex gap-4">
            <div className="w-20 h-20 rounded-lg overflow-hidden shrink-0 bg-slate-100">
              <img src={a.image} alt={a.name} className="w-full h-full object-cover" />
            </div>
            <div className="flex-1 flex flex-col justify-between">
              <div>
                <h4 className="font-bold text-slate-900 text-sm leading-tight">{a.name}</h4>
                <p className="text-xs text-slate-500 mt-1">{a.durationHours} Hours • {a.category}</p>
              </div>
              <div className="flex items-center justify-between mt-2">
                <span className="font-black text-blue-600 text-sm">${a.price}</span>
                <button onClick={() => addAttr(a)} className="text-xs font-bold bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-lg text-slate-700">
                  Add
                </button>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-2">
            {itinerary.attractions.map(a => (
              <div key={a.uniqueId} className="flex justify-between items-center bg-white p-3 rounded-xl border border-slate-200 text-sm">
                <span className="font-bold">{a.name}</span>
                <button onClick={() => {
                  updateItinerary("attractions", itinerary.attractions.filter(item => item.uniqueId !== a.uniqueId));
                }} className="text-red-400 hover:text-red-600">
                  <Trash2 size={16} />
                </button>
              </div>
...
const Step6Meals = ({ itinerary, updateItinerary }) => (
  <div className="text-center py-12 px-4 border-2 border-dashed border-slate-200 rounded-2xl">
    ...
    <p className="text-slate-500 text-sm mt-2">Integrates with daily timeline to specify included vs excluded meals and dietary requirements.</p>
  </div>
);

const Step7Services = ({ itinerary, updateItinerary }) => {
  const addService = (s) => {
    updateItinerary("services", [...itinerary.services, { ...s, price: s.pricePerDay > 0 ? s.pricePerDay * 3 : s.flatFee, uniqueId: Math.random().toString() }]);
  };

  return (
    <div className="flex flex-col gap-4">
      {ADDITIONAL_SERVICES.map(s => (
        <div key={s.id} className="flex justify-between items-center p-4 border border-slate-200 rounded-xl bg-white">
          <span className="font-bold text-slate-700">{s.name}</span>
          <button onClick={() => addService(s)} className="px-4 py-2 bg-slate-100 hover:bg-blue-100 hover:text-blue-700 text-slate-600 text-xs font-bold rounded-lg transition-colors">
            Add Service
          </button>
        </div>
      ))}
      ...
      {itinerary.services.length > 0 && (
        <div className="mt-6">
          ...
          {itinerary.services.map(s => (
             <div key={s.uniqueId} className="flex justify-between items-center bg-white p-3 rounded-xl border border-slate-200 text-sm mb-2">
...
const Step8Timeline = ({ itinerary }) => (
  <div className="text-center py-12 px-4 border-2 border-dashed border-slate-200 rounded-2xl">
    ...
    <p className="text-slate-500 text-sm mt-2 max-w-md mx-auto">This module automatically compiles your selected hotels, transportation, and attractions into an interactive drag-and-drop calendar.</p>
  </div>
);
  ...
  return (
    <div className="bg-slate-50 rounded-2xl p-8 border border-slate-200">
      <div className="flex justify-between items-end mb-8 border-b border-slate-200 pb-6">
        <div>
          <h3 className="text-2xl font-black text-slate-900 tracking-tight">Final Costing</h3>
          <p className="text-slate-500 mt-1">Review internal costs and set client pricing.</p>
        </div>
        <div className="text-right">
          <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Total Net Cost</div>
      ...
      <div className="grid grid-cols-2 gap-6 mb-8">
        <div className="bg-white p-4 rounded-xl border border-slate-200">
          <label className="text-xs font-bold text-slate-400 uppercase mb-2 block">Markup %</label>
        ...
        <div className="bg-blue-600 p-4 rounded-xl border border-blue-700 text-white shadow-lg shadow-blue-500/20">
          <label className="text-xs font-bold text-blue-200 uppercase mb-2 block">Client Selling Price</label>
      ...
      <div className="bg-green-50 text-green-800 p-4 rounded-xl border border-green-200 flex items-center justify-between">
        ...
        <span className="text-xl font-black text-green-700">${(finalPrice - total).toFixed(2)}</span>
      </div>
    </div>
  );
};
