...
export default function AgentDashboardPage() {
  ...
  const kpis = [
    { title: "Total Revenue", value: "$45,231", icon: <CreditCard className="w-5 h-5" />, trend: "+12.5%", positive: true },
    { title: "Active Itineraries", value: "24", icon: <FileText className="w-5 h-5" />, trend: "+4", positive: true },
    { title: "Confirmed Bookings", value: "18", icon: <CheckCircle className="w-5 h-5" />, trend: "+2", positive: true },
    { title: "Pending Quotes", value: "7", icon: <Clock className="w-5 h-5" />, trend: "-1", positive: false },
  ];

  const recentRequests = [
    { id: "REQ-001", client: "The Smith Family", destination: "Malaysia (KL & Langkawi)", dates: "12 Dec - 20 Dec 2026", status: "Draft", budget: "$4,500" },
    { id: "REQ-002", client: "Corp Events LLC", destination: "Dubai", dates: "05 Nov - 10 Nov 2026", status: "Under Review", budget: "$12,000" },
    { id: "REQ-003", client: "Sarah & John", destination: "Bali, Indonesia", dates: "14 Feb - 21 Feb 2027", status: "Quoted", budget: "$3,200" },
    { id: "REQ-004", client: "Williams Group", destination: "Thailand (Phuket)", dates: "10 Jan - 15 Jan 2027", status: "Confirmed", budget: "$8,500" },
  ];
            ...
            {kpis.map((kpi, idx) => (
              <div key={idx} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex flex-col">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${kpi.positive ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                    {kpi.trend}
                  </span>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                <h2 className="text-lg font-bold text-slate-900">Recent Itineraries & Requests</h2>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {recentRequests.map((req, idx) => (
                      <tr key={idx} className="hover:bg-slate-50 transition-colors">
                        <td className="p-4">
                          <div className="text-sm font-bold text-slate-900">{req.client}</div>
                          <div className="text-xs text-slate-500">{req.id}</div>
                        </td>
                        <td className="p-4 text-sm font-medium text-slate-700">{req.destination}</td>
                        <td className="p-4 text-sm text-slate-600">{req.dates}</td>
                        <td className="p-4">
                        ...
                        <td className="p-4 text-sm font-bold text-slate-900">{req.budget}</td>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 flex flex-col">
              <h2 className="text-lg font-bold text-slate-900 mb-6">Upcoming Departures</h2>
              ...
              <div className="flex-1 flex flex-col gap-4">
                <div className="flex items-start gap-4 p-4 rounded-xl border border-slate-100 hover:border-blue-200 hover:shadow-md transition-all cursor-pointer">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-slate-900">Corp Events LLC</h4>
                    <p className="text-xs text-slate-500 mt-1">Dubai • 15 Pax</p>
                ...
                <div className="flex items-start gap-4 p-4 rounded-xl border border-slate-100 hover:border-blue-200 hover:shadow-md transition-all cursor-pointer">
                  <div className="bg-blue-50 text-blue-600 p-3 rounded-lg flex flex-col items-center justify-center min-w-[60px]">
                    ...
                    <span className="text-xl font-black leading-none mt-1">12</span>
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-slate-900">The Smith Family</h4>
                    <p className="text-xs text-slate-500 mt-1">Malaysia • 4 Pax</p>
